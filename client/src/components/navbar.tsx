...
export const Navbar = () => {
  const isAuth = Boolean(useSelector((state: {token: string}) => state.token));
          ...
          {isAuth ? (
            <DropdownMenu>
                <DropdownMenuTrigger asChild>
                    <img className="w-[5rem] h-[5rem] rounded-full p-2 object-cover" src={`http://localhost:3003/assets/${user?.picturePath}`} alt={`${user?.firstName}`} />
                </DropdownMenuTrigger>
            ...
            <nav className="flex items-center space-x-2 lg:space-x-4">
                <a href="/login" className={cn(buttonVariants({ variant: "secondary", size: "sm"}), "px-4 py-2")}>Iniciar sesión</a>
